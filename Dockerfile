...
RUN apk add --no-cache --allow-untrusted wget curl unzip make libtool autoconf automake pkgconfig g++ sqlite sqlite-dev
...
RUN if test "${OPENJPEG_VERSION}" != ""; then ( \
    ...
    && wget --no-check-certificate -q https://github.com/uclouvain/openjpeg/archive/v${OPENJPEG_VERSION}.tar.gz \
...
RUN mkdir proj \
    && apk add --no-cache cmake \
    && wget --no-check-certificate -q https://github.com/OSGeo/PROJ/archive/${PROJ_VERSION}.tar.gz -O - \
...
RUN if test "${GDAL_VERSION}" = "master"; then \
        export GDAL_VERSION=$(curl -k -Ls https://api.github.com/repos/${GDAL_REPOSITORY}/commits/HEAD -H "Accept: application/vnd.github.VERSION.sha"); \
    ...
    && mkdir gdal \
    && wget --no-check-certificate -q https://github.com/${GDAL_REPOSITORY}/archive/${GDAL_VERSION}.tar.gz -O - \
